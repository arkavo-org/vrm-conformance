...
- A **parametric VRM asset generator** that emits a deterministic test corpus. v0.1: full MToon material parameter sweep + single-chain VRMC_springBone parameter sweep (joint count, stiffness, drag, gravity, segment length — all validator-clean). A parallel swing-variant sweep exercises the chains under root-transform excitation, not just gravity settle. Constraints + expressions land in later phases.
...
| `adapters/three-vrm/` | TypeScript adapter for [pixiv/three-vrm](https://github.com/pixiv/three-vrm) (Playwright Chromium WebGL2). |
| `adapters/babylon-vrm/` | TypeScript adapter for [virtual-cast/babylon-vrm-loader](https://github.com/virtual-cast/babylon-vrm-loader) — L1+L2 scaffold; renderer integration deferred to L3. Exists to give consensus diff a third real renderer. |
...
| `test-plans/` | Generated by `vrm-asset-generator emit-sweep`; not committed. |
