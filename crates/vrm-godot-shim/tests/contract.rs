...
fn phase1_ops_render_a_real_vrm() {
    ...
    let calls: Vec<(i64, &str, serde_json::Value)> = vec![
        ...
        (
            2,
            "set_camera",
            serde_json::json!({"position":[0,1.4,1.5],"target":[0,1.4,0],"up":[0,1,0],"fov_degrees":30}),
        ),
        (
            3,
            "set_lighting",
            serde_json::json!({"directional":{"dir":[-0.3,-0.6,-0.7],"color":[1,1,1],"intensity":1},"ambient":{"color":[0.5,0.5,0.5],"intensity":0.3},"cast_shadows":false,"receive_shadows":false}),
        ),
        (
            4,
            "set_post_processing",
            serde_json::json!({"tone_mapping":"None","exposure":1.0}),
        ),
        (
            5,
            "render",
            serde_json::json!({"width":1024,"height":1024,"output_path":out_png.to_string_lossy(),"color_space":"Srgb","msaa":4,"output_type":"Color"}),
        ),
...
fn reserved_ops_still_return_unimplemented() {
    ...
    for (id, method, code, phase) in &cases {
        ...
        assert_eq!(
            ...
            Some(*code),
            "method {method} expected code {code}, got {parsed}"
        );
        if let Some(p) = phase {
            assert_eq!(
                parsed["error"]["data"]["phase"].as_str(),
                Some(*p),
                "phase mismatch for {method}: {parsed}"
            );
        }
    }
    drop(stdin);
...
fn springbone_physics_ops_render_a_real_vrm() {
    ...
    let calls: Vec<(i64, &str, serde_json::Value)> = vec![
        ...
        (
            ...
            serde_json::json!({"translation_start":[0,0,0],"translation_end":[0.15,0,0],"duration_seconds":0.25,"fps":60}),
        ),
        (
            7,
            "step_physics",
            serde_json::json!({"dt_seconds":1.0/60.0,"count":5}),
        ),
        (
            8,
            "render",
...
fn malformed_json_returns_parse_error_with_null_id() {
    ...
    assert!(
        parsed["id"].is_null(),
        "expected null id on parse error, got {parsed}"
    );
