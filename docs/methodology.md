...
- **three-vrm, VMK** — **implement capture** (2026-06-06; reuse their `dump_bone_positions` extraction per `render_sequence` frame, the godot pattern). All four real adapters now capture; see `docs/findings.md` 2026-06-06 (4-renderer comparison): on fast cells VMK matches the oracle on spheres, three-vrm and godot under-deflect; all four penetrate slow. VMK's positions reflect the GPU sim (confirmed by joint lag); three-vrm is glTF-native (no conversion).
...
- Producer quirks are preserved, not patched: the fixtures carry whatever
