...
**The lone genuine open render miss: #226 (MToon rim lighting high-mix).** At `rimLightingMix→1` VMK's rim doesn't fully wrap the unlit silhouette → SSIM **0.949** vs the **0.95** band that `docs/methodology.md:77` deliberately tightens for `mtoon_rimLightingMix_*` (no engine-residual cluster on those tests). Off by 0.001, extreme value only. Last measured at 0.17.0 (`5cd0a95`); the 0.21.0 MToon delta vs 0.20.1 was sub-perceptual (≥0.9974, 2026-06-13 entry), so almost certainly still ~0.949. This is the one cell where VMK trails the golden reference under the suite's own thresholds — a candidate for the tightest-band exception list rather than a release blocker.
