...
**Finding.** Arkavo Spatial's `VRMAGLBWriter` (`ArkavoScan/Export/VRM/VRMAGLBWriter.swift`, as of Spatial commit `c27f6cc` / fixture-export branch `vrma-conformance-fixtures`) emits the hips translation (root-motion) track as a glTF translation channel on a skeleton node that is **NOT** declared in `VRMC_vrm_animation.humanoid.humanBones` unless hips also carries a rotation track. The hips node is present in the glTF animation and drives root motion visually in Spatial's own playback, but its absence from the humanBones map means any spec-conformant VRMC_vrm_animation consumer that retargets exclusively via that map will silently drop it.

**Evidence.** Structural inspection of the committed fixtures:

- `assets/humanoid/spatial_body_motion.vrma` — `humanoid.humanBones` map contains only `head` and `rightUpperArm`; the file has 3 animation channels (2 rotation + 1 hips translation). The hips translation channel targets node 0 (`Hips`) in the glTF node array, but node 0 is absent from `humanBones`.
- `assets/humanoid/spatial_merged_motion.vrma` — `humanoid.humanBones` contains only `rightUpperArm`; 4 channels including 1 hips translation. Same pattern: the hips translation channel is present in the glTF animation but the node it targets is not in `humanBones`.

In both files the hips rotation track (if present) does have a `humanBones` entry; the omission is **specific to the hips translation / root-motion track**.
