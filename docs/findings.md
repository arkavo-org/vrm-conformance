...
## 2026-08-21 (VMK 1.1.0-rc conformance run — release gate) — **`1.1.0-rc` (`9b113ef`) checked against the pin `1.0.0` (`a94531d`): NO conformance regression — the tag PASSES the gate. All 462 synthetic cells, 32 humanoid cells and 2 new AvatarSample_U cells clear their thresholds (0 below gate; the standing `mtoon_firstperson_firstPersonOnly` blank-render rejection fires identically on both sides, as designed — VMK is the conformant renderer there), and every mover is attributable. The 7-commit hop `1.1.0-beta.3..1.1.0-rc` is 449/462 byte-identical, with EVERY spring-bone cell byte-identical — so the 134 spring movers vs `1.0.0` are entirely the beta-line changes already signed off on 2026-08-16 (multigroup collider-group change + #398 solver drift; worst `swing_springbone_joints_16` SSIM 0.98520, reproducing the beta.3 figure exactly). The hop's 13 own movers bisect cleanly: **#420** (perf hot-path) owns 10 — 4 matcap cells (SSIM 0.917–0.981, the only perceptible change anywhere in the release) plus 5 screen-space outline cells and 1 VRMA cell that are sub-perceptual — and **#421** (materialize) owns 3 sub-perceptual VRMA cells; **#417/#418/#419/#392/#390 are render-neutral**. The matcap change was ARBITRATED against three-vrm and moves TOWARD the reference on 3 of 4 cells, so it is not a conformance regression. Two caveats ship with this release: #420's "pixel-identical unless documented" claim does not hold for matcap/outline and should be documented in the release notes, and **[VMK#415](https://github.com/arkavo-org/VRMMetalKit/issues/415) is still open — but blocked on US, not upstream**: the `bothref == secondref` dedup persists at rc, and upstream's 2026-08-17 reply (verified here against three-vrm's shipped source) shows our acceptance test is unreachable in VMK's solver by construction, isolates ORDERING as the real divergence ([VMK#416](https://github.com/arkavo-org/VRMMetalKit/issues/416), 5.63e-02 m vs our 0), and asks us to decide whether to close #415 in its favour — unanswered for four days. Do NOT pin (pre-release; policy) — `Package.swift` stays at `1.0.0` pending the stable `1.1.0` tag and explicit sign-off.**

**Method.** Clean self-contained adapter builds per CONTRIBUTING (`rm -rf .build`, then stash `vrm-metal-kit-adapter` **together with** `VRMMetalKit_VRMMetalKit.bundle`) at `1.0.0` (`a94531d`), `1.1.0-beta.3` (`064a891`) and `1.1.0-rc` (`9b113ef`), plus three bisect builds at `0ecf378`, `0507ac6`, `6c34e7c`. Isolation proven on every binary: the collider probe (`..._r0p05` vs `..._r0p1`) differs on all six, and the `1.0.0` side byte-reproduces **382/428** cells of the June goldens cache — the 46 movers are exactly the documented 0.21.0 + augment-off staleness (pbrtex/rimtex/shadetex/uvxform + the r0p1-radius collider cells), i.e. no frankenstein pairing. Corpus: **462 synthetic plans** (MToon sweep + emissive / emissive-strength / outline-colour-lighting-mix / first-person / texture-transform / shadetex / matcap / shading-shift-tex / rimtex / outline-width-tex / pbrtex sweeps + settle / swing / collider / multigroup / extended / taper / gravity-dir / multichain / leaftail / coupling spring sweeps, all spring plans `augment_colliders: false`, + 7 VRMA sweeps: humanoid / expression / lookAt / ARKit / multichannel / finger / hips) plus the **32-plan humanoid manual corpus** (augment-on) and **2 ad-hoc AvatarSample_U 1.0 cells** (below). A/B = fresh same-machine renders both sides; byte-compare first, SSIM only on movers.

**Attribution — the 7-commit hop bisects to two commits.**

| commit | scope | movers introduced |
| --- | --- | --- |
| `ecddc47` #417 orthographic projection clipped every fragment | renderer fix | **0** — structurally unverifiable (see gaps) |
| `874ff80` #418 per-group collider application test (answers our VMK#415) | test-side | 0 |
| `0ecf378` #419 loader transform jitter tolerance | test-side | 0 |
| `0507ac6` #420 perf: hot-path groups 1–6, compositor submit, visionOS host | renderer | **10** — 4 matcap, 5 screen-space outline, 1 VRMA |
| `6c34e7c` #421 materialize: progress-driven spawn effects, 12 styles | feature | **3** — VRMA cells, all sub-perceptual |
| `bdc100d` #392 Swift 7 upcoming features | build | 0 |
| `9b113ef` #390 declare visionOS 26+ support | docs | 0 |

`#421`'s spawn effects are confirmed **default-off**: the other 127 cells in the bisect render set are byte-identical, so a default-on effect is not contaminating frame-0 renders. Its 3 movers are `vrma_humanoid_head_pitch_20`, `vrma_humanoid_head_roll_15` and `vrma_multi_all_channels` — SSIM 1.00000, max Δ 30/1/34 over <0.01 % of pixels.

**The matcap change is the only perceptible mover — and it improves agreement with the reference.** #420's four matcap cells move materially (`mtoon_matcap_default` SSIM 0.91696, `..._dim` 0.92893, `..._red_tint` 0.93144, `..._blue_tint` 0.98061; up to 14 % of pixels changed, 8.7 % above Δ 32). `mtoon_matcap_baseline` (no matcap texture) is byte-identical, so the change is confined to the matcap path. Rendering the matcap corpus through **three-vrm** as arbiter:

| cell | SSIM vs three-vrm @ `1.0.0` | @ `1.1.0-rc` | Δ |
| --- | --- | --- | --- |
| `mtoon_matcap_baseline` | 0.87221 | 0.87221 | ±0 |
| `mtoon_matcap_default` | 0.84199 | 0.85802 | **+0.01603** |
| `mtoon_matcap_dim` | 0.86490 | 0.87599 | **+0.01109** |
| `mtoon_matcap_red_tint` | 0.73953 | 0.74252 | **+0.00299** |
| `mtoon_matcap_blue_tint` | 0.86834 | 0.86315 | −0.00519 |

rc moves toward three-vrm on 3 of 4 active cells and marginally away on one — a net improvement, not a regression. (The low absolute SSIMs are the pre-existing cross-renderer matcap divergence, unchanged by this run.) Delta structure: broad contiguous coverage of the whole matcap-shaded surface with a **~zero signed mean (+0.06) and p99 |Δ| = 104** — a redistribution consistent with a shift in the matcap lookup coordinate, not a brightness shift or texture-compression artefact. #420 both splits the vertex streams (group 6b) and merges the MToon surface + inverted-hull outline into one instanced draw (group 5); the exact mechanism was not isolated further. The 5 screen-space outline movers (`mtoon_outline_screen_*`, `mtoon_outlinewidthtex_screen`) are the same commit's outline work: SSIM ≥ 0.99995, max Δ 137–188 but ≤ 0.01 % of pixels — a handful of silhouette-edge pixels.

**Upstream claim vs observation.** #420's PR text states "Pixel-identical unless documented." That holds for 452/462 cells but **not** for the matcap cells (down to SSIM 0.917) or the screen-space outline cells. Neither is a spec regression — matcap in fact improves — but the 1.1.0 release notes should document both so downstream golden-holders re-baseline deliberately.

**Real-avatar corpus — the hop is near-neutral, unlike beta.2→beta.3.** 18/32 humanoid cells are byte-identical beta.3→rc; all 14 movers are sub-perceptual (SSIM ≥ 0.99992, RMSE ≤ 0.28, ≤ 0.02 % of pixels changed, **none above Δ 32**) and concentrate on the spring-coupled cells (`avatarA_bosom*`, `vroid_default_F_bust_*`, `..._collider_settle`, `..._spatial_*`, the `_bodyL`/`_bodyR` gaze cells). This is two orders of magnitude smaller than the beta.2→beta.3 hop (RMSE up to 19.3), i.e. the rc hop does not repeat the texture-pipeline shift.

**#420's AvatarSample_U skin-palette claim — partially closed, and a broken fixture found.** #420 claims "skin palette 1024 and sentinel-only `JOINTS_0` sanitize so Avatar U's right shin/foot (palette 257+) no longer remaps to joint 0" — a real rendering fix the synthetic corpus cannot see. `assets/humanoid/avatarU_0_0.vrm` turns out to be a **stale symlink** to `AvatarSample_U_0.0.vrm.glb`, which no longer exists in the VRMMetalKit checkout (only `AvatarSample_U_1.0.vrm.glb` does), so the standing `avatarU_0_0` cell has been failing with "asset not found" on both sides — pre-existing, not a regression, but it means that cell has been contributing nothing. Two ad-hoc `AvatarSample_U` 1.0 cells (full-body + legs framing) were rendered at all three revisions instead: **both shins and both feet render correctly at `1.0.0`, `beta.3` and `rc`** — no joint-0 collapse on the 1.0 sample, so the fix's before-state does not reproduce here and the claim remains unverified rather than confirmed. The beta.3→rc delta on these cells is small and *not* in the legs: full-body SSIM 0.99971, legs 0.99760, with 98.7 % of changed pixels in the top third of the legs framing (the skirt hem / tie ornament — spring-driven accessories), 0.75 % of pixels and 0.07 % above Δ 32.

**VMK#415 (multigroup collider dedup) is still open at rc.** The discriminator, re-measured on all three self-contained binaries with collider engagement proven on each:

| revision | `bothref` vs `secondref` (settle) | (swing) |
| --- | --- | --- |
| `1.0.0` (`a94531d`) | differ | differ |
| `1.1.0-beta.3` (`064a891`) | **identical** (dedup) | **identical** |
| `1.1.0-rc` (`9b113ef`) | **identical** (dedup) | **identical** |

#418 responds to the issue but is test-side and does not change render output — the acceptance test filed in VMK#415 (`bothref != secondref`) has **not** flipped. **But that acceptance test turns out to be invalid, and #415 is now blocked on us, not on upstream.** Upstream's reply (2026-08-17) refutes the mechanism with measurements, and it is correct — verified here against primary source (`@pixiv/three-vrm-springbone` 3.5.0 shipped lib, `_collision()`):

```js
for (let cg = 0; cg < this.colliderGroups.length; cg++) {         // group-major, no dedup — our premise was right
  for (let c = 0; c < this.colliderGroups[cg].colliders.length; c++) {
    ...
    if (dist < 0) {
      tail.addScaledVector(_v3A7, -dist);                          // push out
      tail.sub(_worldSpacePosition);
      const length = tail.length();
      tail.multiplyScalar(this._worldSpaceBoneLength / length)      // <- in-loop length re-projection
          .add(_worldSpacePosition);
    }
```

The references genuinely do not dedup (UniVRM `.SelectMany(group => group.Colliders)` with no `.Distinct()`; three-vrm the nested loop above). But the **in-loop length re-projection after each hit** is what makes a second application of the same collider do work — it pulls the tail off the collider surface and generally back into it. VMK has no such step: collision is a projection onto the collider surface at `responseScale 1.0`, and the distance constraint is a separate kernel dispatched *before* collision, never between two colliders in a pass. So in VMK repeating a push-out is idempotent — upstream measures **1.49e-08 m** (float epsilon) between today's dedup upload and the exact per-group upload the issue requests. **Our discriminator asset carries a single collider, and with one collider group-major and collider-major orderings coincide**, so `bothref != secondref` is unreachable in this solver regardless of how the fix is ordered. The discriminator did catch a real divergence (the `1.0.0` drop bug, correctly fixed); what it cannot do is serve as the acceptance test for the per-group change.

**The real divergence upstream isolated is ordering, and it is much larger than what we filed.** Same scene, two overlapping colliders, `X` in both referenced groups and `Y` in the first only: per-group *collider-major* is inert (9.3e-09 m), per-group *group-major* — UniVRM's order — moves the joint **5.63e-02 m**. UniVRM/three-vrm evaluate a per-spring group-major collider list; VMK uploads one global buffer in authored collider order and filters per bone with a 32-bit group mask. Their exposure census (co-reachable collider pairs whose rest-pose bounds intersect) finds 7.4–9.9 % of pairs overlapping and **every spring on every sampled avatar** (AvatarSample_U/A/K, vroid_default_F) carrying at least one — reachable, not a corner case. Filed upstream as [VMK#416](https://github.com/arkavo-org/VRMMetalKit/issues/416).

**Status and what the suite owes.** #415 is OPEN only because upstream proposed closing it in favour of #416 *and asked us to decide* — that request has been unanswered since 2026-08-17. Position: accept the refutation (the dedup framing is dead; the fix as specified is provably inert and would shift the sphere-array indices the public `sphereColliderRadiusOverrides` API is keyed on), agree to close #415 in favour of #416, and **build the discriminator #416 actually needs** — two *overlapping* colliders in different groups referenced by one spring, with the authored order deliberately disagreeing with the group-major order, measured against the UniVRM oracle. Per the address-the-whole-gap rule that is a sweep, not one cell: overlap depth × authored-vs-group order × shape pairing. **Do not grant upstream's proposal to pre-classify the residual `bothref` cohort gap as a "documented collision-integration methodology difference" yet** — per the project's decision pattern (test the spec; divergence is the deliverable; don't soften failure flags with methodology framing), that classification should follow the ordering measurement, not precede it.

**Coverage gaps (flagged, not silently passed).**
1. **#417 orthographic projection is structurally unverifiable on this corpus** — every plan uses a perspective camera (`fov_degrees`), so a fix to the orthographic path can only ever render neutral here. The commit is credited as render-neutral for the *gate*, not as *verified*. Follow-up: an orthographic-camera plan variant, per the address-the-whole-gap rule.
2. **#421's spawn effects (12 styles, progress-driven) are unexercised** — the corpus only ever renders the default (effect off), which is what makes the neutrality result meaningful but leaves the feature itself untested.
3. **`avatarU_0_0` fixture is broken** — repair the symlink or repoint the plan at `AvatarSample_U_1.0.vrm.glb` (and add the legs framing, which is where U's palette-257+ geometry actually sits).
4. The beta.3 texture-pipeline gap (sampler sweep: omitted `minFilter`, mipmapped min filters, CLAMP/MIRROR wrap; textured-soft-alpha MToon) **still stands** — untouched by this run.

**SIGN-OFF (regression-check gate, per the standing pin-advance convention).** **GO** — I sign off on promoting `1.1.0-rc` (`9b113ef`) to stable `1.1.0`. Basis: no conformance regression vs the `1.0.0` pin across 462 synthetic + 32 humanoid + 2 AvatarSample_U cells, 0 below gate, every mover attributed to a named commit, and the one perceptible change (matcap, #420) arbitrated against three-vrm as moving *toward* the reference. **Condition:** the 1.1.0 release notes must document the matcap and screen-space-outline deltas, since #420's own PR text claims "pixel-identical unless documented" and that claim does not hold for those 9 cells — downstream golden-holders need to re-baseline deliberately rather than discover it.

**Scope of the sign-off — what it does NOT cover.** "No conformance regression" is not "everything in this release is verified." Three claims in the range are outside what this corpus can attest to, and the sign-off is silent on them rather than endorsing them: (1) **#417's orthographic-projection fix** — structurally unverifiable, every plan uses a perspective camera; (2) **#421's materialize spawn effects (12 styles)** — a new feature surface the corpus never exercises, verified only to be default-off; (3) **#420's AvatarSample_U skin-palette claim** — the before-state does not reproduce on the 1.0 sample and the 0.x fixture is broken, so it is unverified rather than confirmed. Separately, the pin advance is a **distinct gate**: `Package.swift` stays at `1.0.0` until stable `1.1.0` is tagged (never pin a pre-release), and adopting it then requires a deliberate re-baseline of the matcap (4), screen-space-outline (5) and spring-bone (134) goldens.

**Pointers.** Upstream: `1.1.0-rc` (`9b113ef`), range `1.1.0-beta.3..1.1.0-rc` = 7 commits (#417, #418, #419, #420, #421, #392, #390). Suite-side: **no pin change** — `adapters/vrm-metal-kit/Package.swift` stays at `1.0.0` (`a94531d`); advance to stable `1.1.0` only after that tag exists and the pin-advance sign-off is given, at which point the matcap + screen-space-outline + spring-bone goldens all need a deliberate re-baseline. Related entries: 2026-08-16 (beta.3 check, #398 bisect, SPM bundle-fallback hazard), 2026-07-17 (beta.1 check, multigroup discriminator, UniVRM arbitration), `docs/methodology.md` spring-bone augmentation convention.

## 2026-08-16 (VMK 1.1.0-beta.3 regression check) — **range `c68eb28..064a891` (beta.1→beta.3, ~24 commits) checked against the pin `1.0.0` (`a94531d`): NO conformance regression. Every non-spring cell is byte-identical (124/125; the lone mover is `mtoon_giEqualization_0p5` at max Δ = 1, ±1 LSB — sub-perceptual); #366's "pixel-identical" rim-specialization claim is VERIFIED byte-exactly on all 10 non-zero-rim cells; the beta line's only spring-bone *pixel* change vs 1.0.0 (92 spring cells) splits cleanly into (a) the already-known multigroup collider-group fix (beta.1, the two `secondref` cells) and (b) a global solver drift introduced in the beta.2→beta.3 hop — **bisected to #398** (per-substep host-write segmentation): a build at `8d87fd5` (= beta.2 + #398 alone) reproduces beta.3's spring renders byte-identically 92/92, so #365 and every other commit in the hop are spring-render-neutral. Worst #398 cell `swing_springbone_joints_16` SSIM 0.9852 vs 1.0.0; all cells clear the 0.85 gate. The beta's `bothref` dedup (the UniVRM-oracle divergence from the 2026-07-17 arbitration) PERSISTS at beta.3 — now re-measured with collider engagement proven on the same binary, so the standing upstream note (match the reference's per-group application) still applies. Do NOT pin (pre-release; policy). Separately: this run surfaced and fixed a serious measurement hazard (SPM resource-bundle fallback, below) that invalidated two full A/B passes before the third, valid one — and re-validated the entire 2026-07-17 measurement set in the process.**

**Method.** Clean self-contained adapter builds (`rm -rf .build`, then stash `vrm-metal-kit-adapter` **together with** `VRMMetalKit_VRMMetalKit.bundle`) at `1.0.0` (`a94531d`), `1.1.0-beta` (`c68eb28`), `1.1.0-beta.2` (`e2534cc`), and `1.1.0-beta.3` (`064a891`). Corpus: 217 synthetic plans (MToon sweep + emissive/outline/first-person/texture-transform/shadetex/matcap/shading-shift-tex/rimtex/outline-width-tex/pbrtex sweeps + settle/swing spring-bone + collider sweep + multigroup discriminator, all spring plans `augment_colliders: false`) plus the 32-plan humanoid manual corpus (augment-on — plans carry no flag). A/B = fresh same-machine renders both sides; byte-compare first, SSIM only on movers. Determinism cross-check: two independent clean builds of beta.3 produced 217/217 byte-identical renders.

**Measurement hazard found: SPM `Bundle.module` fallback makes a bare copied adapter binary a frankenstein renderer.** SwiftPM's generated resource accessor loads `VRMMetalKit_VRMMetalKit.bundle` (the metallibs) from the executable-adjacent path if present, else from the **absolute baked `.build` path** — i.e. whatever revision was built last. Copying only the executable out of `.build` for an A/B run therefore pairs one revision's host code with another revision's shaders once `.build` is rebuilt. Observed artifacts before diagnosis (both passes discarded): a phantom "rim regression" (1.0.0 host never sets beta.3's new `fc_hasParametricRim` function constant → rim ALU dead-stripped → RMSE up to 43 on all rim cells) and phantom "authored colliders inert" (radius/position sweeps byte-identical). Proof of mechanism: the same copied 1.0.0 executable renders collider cells engaged and **byte-identical to the June cache** when `.build` holds 1.0.0, and inert when `.build` holds beta.3; with the bundle stashed adjacent, isolation holds regardless of `.build` state. **Rule (now in CONTRIBUTING):** stash exe + bundle together per side; treat a sub-second `swift build` after a pin change as a red flag; sanity-probe each binary (collider `r0p05` vs `r0p1` must differ; a known cell must byte-match `goldens-cache-rc`).

**2026-07-17 measurements re-validated, and strengthened.** The hazard initially cast doubt on the July entries; clean re-measurement instead confirms all of them: (1) the July "1.0.0" multigroup table reproduces **exactly** (`singleref != secondref`, `singleref == bothref` — secondref dropped, bothref applied once); (2) `c68eb28`'s dedup (`singleref == secondref == bothref`) reproduces on a self-contained build **with engagement proven on the same binary** (`r0p05` vs `r0p1` differ), so it was genuine dedup, not dead colliders; (3) the June-11 cache is byte-reproducible today at 0.21.0, rc.2, and 1.0.0 — which upgrades the rc.2→1.0.0 render-neutrality claim from "by construction" to **verified on collider cells** (#369 really is render-only); (4) the r0p1-radius collider cells' cache deltas are exactly the augment-off methodology change (augment-ON re-render == cache byte-identically).

**Beta.2→beta.3 on the humanoid corpus (the thirteen-change hop the release notes describe).** All 32 humanoid cells move, and the release notes' guidance is confirmed in kind but understated in magnitude for animated/physics content: the static-pose cells show the notes' texture-pipeline signature spread wide (`vroid_default_F_gaze_center`: 24.8% of pixels changed but 22.9% sit in the 5–32 delta band, only 1.3% above — the broad, low-amplitude shift expected from straight-alpha decode #400/#401 + sampler-driven mip chains #402/#411 on real VRoid textures), while the physics-coupled cells move much more than any rest-pose diff shows (`vroid_default_F_bust_swing` RMSE 19.3 with 3.1% of pixels above Δ32 — that is #398 acting on real hair/bust chains, invisible in a static 4-asset comparison). The full 1.0.0→beta.3 humanoid delta is dominated by the beta line's default-on synthetic-collider expansion (beta.1 torso/upper-arm: an identical rigid shift on every expression cell, RMSE 39.1; beta.2 shoulder/breast colliders on top), i.e. augment-on humanoid renders measure progressively more VMK-proprietary physics — reinforcing the augment-off methodology decision for spec-faithful comparison.

**Coverage gap flagged (unchanged by this run, now explicit).** The synthetic corpus is texture-pipeline-invariant *by design*: alpha cells use `baseColorFactor` alpha (never decoded from an image), samplers are uniformly LINEAR/no-mip/REPEAT, and 16×16 textures render magnified — so #400/#401/#402/#411 are structurally unverifiable on it (and indeed 216/217 cells are byte-identical across the hop that changed all four). The humanoid cohort is currently the only evidence source. Follow-up: a sampler sweep (omitted `minFilter`, mipmapped min filters, CLAMP/MIRROR wrap) and a textured-soft-alpha MToon variant, per the address-the-whole-gap rule.

**Pointers.** Upstream: `1.1.0-beta.3` release notes (064a891; thirteen changes since beta.2, closes #406). Suite-side: no pin change (`Package.swift` stays at `1.0.0`); CONTRIBUTING gains the exe+bundle stash rule. Standing upstream note from 2026-07-17 (beta dedups `bothref`; oracle applies per-group) re-confirmed at beta.3 and now FILED as [VMK#415](https://github.com/arkavo-org/VRMMetalKit/issues/415) (2026-08-16), with the discriminator table as repro and the `bothref != secondref` flip as the acceptance test. Related entries: 2026-07-17 (beta.1 check, multigroup discriminator, arbitration), `docs/methodology.md` spring-bone augmentation convention.
