...
## 2026-06-14 (benchmark structural divergence: default asset draw-call + triangle count) — **RESOLVED (methodology): outlines are OFF on the default asset; VMK and three-vrm correctly render 1 draw call / 2304 triangles. UniVRM's 3 draw calls / 4610 triangles is a Built-in RP metric-semantics artifact — it counts GPU-submitted geometry from an always-dispatched (but `clip(-1)`-discarded) MToon outline pass plus the always-compiled ShadowCaster pass. NOT a VMK/three-vrm fidelity gap. The initial "VMK/three-vrm differ from UniVRM" hypothesis is REVERSED.**
...
All three used the identical asset, identical scene config (camera / lighting / post from the asset's generated `test.yaml`), and `tone_mapping: none`. `perf-report.sh` reports VMK `draw_calls` −66.67 % vs the UniVRM reference.

**Signal source.** The structural/geometry layers are hardware-independent and deterministic — explicitly **not** the timing layer. Numbers reproduce from the per-test `*.perf.json` (`vrm-runner benchmark-execute` for VMK/three-vrm; `execute-test-batch --benchmark` for UniVRM).

**Root cause (confirmed by code inspection).** The default asset has **outlines OFF**: `MToonParams::defaults()` ships `outlineWidthMode: "none"`, `outlineWidthFactor: 0.0` (`crates/vrm-asset-generator/src/params.rs:160-163`; verified in the generated GLB's `VRMC_materials_mtoon`). Base mesh = 2304 triangles / 1225 vertices.

- **VMK and three-vrm are CORRECT.** Both gate the outline pass on outline mode at the CPU level and skip it entirely when mode is `none`: VMK `renderMToonOutlines()` guards `mtoon.outlineWidthMode != .none && outlineWidthFactor > 0.0001` (`adapters/vrm-metal-kit/.build/checkouts/VRMMetalKit/Sources/VRMMetalKit/Renderer/VRMRenderer.swift:3866-3871`); three-vrm's `@pixiv/three-vrm-materials-mtoon` only creates the outline material clone when `outlineWidthMode !== "none" && outlineWidthFactor > 0`. Result: 1 draw call, 2304 triangles = the real visible geometry.
- **UniVRM over-submits (Built-in RP shader artifact).** The Built-in RP MToon10 shader (`…/PackageCache/com.vrmc.vrm@…/MToon10/Shaders/vrmc_materials_mtoon.shader:107-144`) hardcodes a `FORWARD_BASE_OUTLINE` pass into the SubShader that is **always dispatched**. When `outlineWidthMode = none`, the geometry vertex shader collapses all verts toward the origin (`positionOS * 0.001`, `…_geometry_vertex.hlsl:85-91`) and the fragment shader `clip(-1)`s every pixel (`…_forward_fragment.hlsl:17-19`) — zero visible output, but the GPU still receives the draw call and 2304 triangles. A third draw call is the always-compiled `ShadowCaster` pass (present regardless of the plan's `cast_shadows: false`, which controls scene config, not shader pass compilation). UniVRM's benchmark reads `UnityEditor.UnityStats.*`, which counts **GPU-submitted** geometry, not rasterized/visible geometry. Hence 3 draw calls and ≈2× triangles (2304 base + 2304 discarded outline + shadow), none of which represents extra visible rendering.

**Verdict.** The divergence is **not** a VMK/three-vrm conformance gap and **not** UniVRM rendering visible extra geometry. It is a **benchmark metric-semantics difference**: UniVRM's `draw_calls`/`triangles` (via `UnityStats`) count submitted-but-discarded passes, while VMK/three-vrm count the work they actually issue. For MToon assets with outlines off, expect UniVRM to report ~2× triangles + ~2 extra draw calls of phantom (clipped) geometry. The `−66.67 %` `draw_calls` delta is metric noise, not an efficiency or fidelity divergence.

**Implications.** (1) **Methodology:** cross-renderer `draw_calls`/`triangles` comparison is not apples-to-apples against UniVRM Built-in RP — its counts include always-submitted outline + ShadowCaster passes. A future refinement could have the UniVRM adapter subtract the clipped outline pass when `outlineWidthMode == none`, or annotate the measurement; until then, interpret UniVRM structural counts with this caveat (noted for `docs/methodology.md`). (2) **Phase 2 (budgets/gating):** a "familiar band" threshold on `draw_calls`/`triangles` must account for this UniVRM offset, or normalize it out — do not codify the 66 % gap as a real divergence. (3) **Re-test with outlines ON** (an `emit-sweep` outline variant) would be the clean confirmation that all three render the outline when the asset enables it; expected outcome: VMK/three-vrm rise to ~2 draw calls / ~2× triangles, UniVRM unchanged.

**Discovery method.** Surfaced by the new `benchmark_execute` op (performance-metrics slice, 2026-06-14); root-caused by inspecting the asset's MToon defaults, the UniVRM Built-in RP shader passes, and the VMK/three-vrm outline guards. First cross-renderer structural divergence produced by the benchmark infra — and a reminder that "divergence" can be a metric artifact, which the root-cause step is there to catch.
