...
## 2026-08-16 (VMK 1.1.0-beta.3 regression check) — **range `c68eb28..064a891` (beta.1→beta.3, ~24 commits) checked against the pin `1.0.0` (`a94531d`): NO conformance regression. MToon is 216/217 byte-identical (the lone mover is `mtoon_giEqualization_0p5` at max Δ = 1, ±1 LSB — sub-perceptual); #366's "pixel-identical" rim-specialization claim is VERIFIED byte-exactly on all 10 non-zero-rim cells; the beta line's only spring-bone *pixel* change vs 1.0.0 splits cleanly into (a) the already-known multigroup collider-group fix (beta.1, the two `secondref` cells) and (b) a global solver drift introduced in the beta.2→beta.3 hop — attributable to #398 (per-substep host-write segmentation), worst cell `swing_springbone_joints_16` SSIM 0.9852 vs 1.0.0, all cells clear the 0.85 gate. The beta's `bothref` dedup (the UniVRM-oracle divergence from the 2026-07-17 arbitration) PERSISTS at beta.3 — now re-measured with collider engagement proven on the same binary, so the standing upstream note (match the reference's per-group application) still applies. Do NOT pin (pre-release; policy). Separately: this run surfaced and fixed a serious measurement hazard (SPM resource-bundle fallback, below) that invalidated two full A/B passes before the third, valid one — and re-validated the entire 2026-07-17 measurement set in the process.**
