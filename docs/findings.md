...
**Remedy / next steps.** (1) **Reduce the default outline width** (e.g. `0.01`–`0.02` world, or `screenCoordinates`) so the default renders a shaded avatar + a *thin* outline ring — restoring informative MToon-shading goldens and shrinking the outline to a region the methodology's outline-region-local SSIM tolerance can handle. (2) Re-assess VMK's outline extrusion vs UniVRM at that sane width (likely a minor ring difference; a VRMMetalKit fidelity question, not the 0.80-SSIM whole-avatar gap). (3) `consensus-report` should apply the outline-region-local tolerance, not a global 0.85 threshold, for outlined cells. (4) **godot's broad divergence** (SSIM ~0.39 across the board) is a separate, larger L4-partial-adapter gap, not outline-specific.
