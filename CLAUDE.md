...
- `adapters/vrm-metal-kit/` — Swift / Metal / macOS 26 platform floor. Pins a specific VRMMetalKit upstream revision in `Package.swift`; bump that revision deliberately as part of the change. **`render_sequence` real** (RFC-0004 Phase 5; PNG + animate_root_transform; apply_vrma + MP4/MOV mux deferred). **`benchmark_plan`/`benchmark_execute` real** (RFC perf-v1; full `PerfReport` — frame-time p50/p95/p99 + FPS, draw calls/state changes/texture bindings, triangle/vertex counts, peak GPU memory + load/first-frame ms — via VRMMetalKit `PerformanceTracker` + `device.currentAllocatedSize`; runtime coverage local-only like the other VMK ops). CI builds (debug) but does not test (Xcode 26 binaries link against macOS 26 libs absent on `macos-15` runners — runtime coverage runs locally on M-series Macs / Xcode Cloud).
- `adapters/three-vrm/` — Phase 1 ops real; Playwright headless Chromium running three.js + three-vrm. **`render_sequence` real** (RFC-0004 Phase 6; PNG + animate_root_transform; apply_vrma deferred). **`benchmark_plan`/`benchmark_execute` real** (CPU frame-time p50/p95/p99 + FPS, `draw_calls`, `triangles`, host JS-heap memory; `state_changes`/`texture_bindings`/`vertices` omitted — three.js does not instrument them; headless Chromium GPU is software so timing is indicative, structural/geometry are the cross-renderer signal). Other reserved ops still return `Unimplemented`. Requires `npx playwright install chromium` after `npm install`.
...
- `adapters/godot-vrm/` — Godot 4 / GDScript paired with the `crates/vrm-godot-shim/` Rust shim. L4 (Phase 1 + spring-bone physics real). Full 80-test corpus renders end-to-end. **`render_sequence` real** (RFC-0004 Phase 6; PNG + animate_root_transform; apply_vrma deferred). Runner consumes `target/release/vrm-godot-shim` as `--adapter-bin`. Requires Godot 4.x on `PATH`.
- `adapters/univrm/` — Unity 6 / UniVRM v0.131.0 / Built-in RP. L4 PlayMode — Phase 1 ops real (44 MToon variants + 36 spring-bone tests). Spring-bone tests render with real physics in PlayMode (FastSpringBone gates on `Application.isPlaying`); EditMode (`L3_EDITMODE=1`) renders rest pose. **`render_sequence` real in PlayMode** (RFC-0004 Phase 7; PNG + animate_root_transform; apply_vrma + MP4/MOV mux deferred). EditMode rejects sequence plans with a clear "requires PlayMode batch" error. **`benchmark` via `execute-test-batch --benchmark` real** (PlayMode batch warmup+measured loop; CPU frame-time p50/p95/p99 + FPS, `draw_calls`, `triangles`, `vertices`, host memory; `state_changes`/`texture_bindings` omitted — Unity doesn't expose them; UnityStats EditMode-gated). Uses **batched one-shot via filesystem** (RFC-0003); runner consumes `adapters/univrm/launcher.sh` as `--adapter-bin` via `execute-test-batch`. Requires Unity 6000.4.6f1 + Personal license activated locally; CI does build-validate only.

### `render_sequence` (RFC-0004)

Multi-frame capture for temporal conformance. End-to-end across all four real adapters + the deterministic mock. Test plans opt in with a `render_sequence:` block (mutually exclusive with `animation:` per `TestPlan::validate`). The runner dispatches `render_sequence` instead of single-frame `render`; per-frame PNGs land at `<output_dir>/<test_id>_<renderer>_frames/<NNNN>.png`. BLAKE3 is centralized in the runner — adapters return a 64-zero sentinel and the runner re-hashes from on-disk PNG bytes (`execute.rs::rehash_frames` for per-op adapters, batch-level loop in `execute_batch.rs::run` for UniVRM). Diff via `vrm_diff_engine::temporal::temporal_diff` (mean/p95/min SSIM + worst-frame + BLAKE3 short-circuit). Asset corpus via `cargo run -p vrm-asset-generator -- emit-sequence-sweep` (20 swing variants, `swing_seq_*` prefix). See [`rfcs/0004-render-sequence-op.md`](rfcs/0004-render-sequence-op.md) (Accepted) and the rollout meta-plan at `docs/superpowers/plans/2026-05-18-render-sequence-rollout.md`.
