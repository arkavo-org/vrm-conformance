...
- `adapters/three-vrm/` — Phase 1 ops real; Playwright headless Chromium running three.js + three-vrm. **`render_sequence` real** (RFC-0004 Phase 6; PNG + animate_root_transform; apply_vrma deferred). Other reserved ops still return `Unimplemented`. Requires `npx playwright install chromium` after `npm install`.
