...
- `adapters/vrm-metal-kit/` — Swift / Metal / macOS 26 platform floor. Pins a specific VRMMetalKit upstream revision in `Package.swift`; bump that revision deliberately as part of the change. **`render_sequence` real** (RFC-0004 Phase 5; PNG + animate_root_transform; apply_vrma + MP4/MOV mux deferred). CI builds (debug) but does not test (Xcode 26 binaries link against macOS 26 libs absent on `macos-15` runners — runtime coverage runs locally on M-series Macs / Xcode Cloud).
